// Towers of Hanoi (Even)- Steven Dorvil
#include <iostream>
#include<vector>
...
int main() {
  ...
  while(t[1].size() < n+1) {
    ...
    if (to == 0) {
      if (t[abs((to-2)%3)].back() < t[abs((to+1)%3)].back()) {
        from = abs((to-2)%3);
      } else {
        from = abs((to+1)%3);
      }
    } else if (t[abs((to-1)%3)].back() < t[abs((to+1)%3)].back()) {
      from = abs((to-1)%3);
    } else {
      from = abs((to+1)%3);
    }
    ...
    // change the to tower
    if (from == 0) {
      if (t[from].back() < t[abs((from-2)%3)].back())
        to = abs((from-2)%3);
      else
        to = abs((from+1)%3);
    } else {
      if (t[from].back() < t[abs((from-1)%3)].back())
        to = abs((from-1)%3);
      else
        to = abs((from+1)%3);
    }
